...
pub struct DeployOpts {
    ...
    /// Do not upload keys
    ///
    /// By default, Navi will upload secret keys set in `deployment.keys` before deploying
    /// the new profile on a node. To upload keys without building or deploying the rest
    /// of the configuration, use `navi upload-keys`.
    #[arg(long)]
    no_keys: bool,
    ...
    /// not on the host running Navi. This overrides per-node preferences set in
    ...
    /// Install bootloader
    ///
    /// If enabled, the bootloader will be installed during activation.
    /// This is equivalent to `NIXOS_INSTALL_BOOTLOADER=1`.
    #[arg(long)]
...
pub async fn run(hive: Hive, opts: Opts) -> Result<(), NaviError> {
    ...
    let options = {
        ...
        options.set_install_bootloader(install_bootloader);
