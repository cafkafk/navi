...
pub struct Opts {
    ...
    /// Do not deploy keys
    ///
    /// Do not deploy secret keys set in `deployment.keys`. By default, Navi will deploy keys
...
pub async fn run(
...

    {
        ...
        if euid != 0 && !sudo {
            tracing::warn!("Navi was not started by root. This is probably not going to work.");
