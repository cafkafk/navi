...
pub struct Opts {
    ...
    /// Command to run
    ///
    /// It's recommended to use -- to separate Navi options from the command to run. For
    /// example:
    ///
...
pub async fn run(
    ...
    let meta = meta.run(|meta| async move {
        ...
        let results: Vec<Result<(), NaviError>> = join_all(futures).await;
    ...
    if failed > 0 {
        Err(NaviError::ExecError { n_hosts: failed })
    } else {
        Ok(())
    }
}
