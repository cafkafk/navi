...
pub enum EventPayload {
    ...
    /// The job failed.
    ///
    /// We can't pass the NaviError because the wrapper needs to
...
impl JobMonitor {
    ...
    /// Starts the monitor.
    pub async fn run_until_completion(mut self) -> NaviResult<Self> {
    ...
    async fn finish(mut self) -> NaviResult<Self> {
...
impl JobHandleInner {
    ...
    pub fn create_job(&self, job_type: JobType, nodes: Vec<NodeName>) -> NaviResult<JobHandle> {
    ...
    pub async fn run<F, U, T>(self: Arc<Self>, f: U) -> NaviResult<T>
    where
        U: FnOnce(Arc<Self>) -> F,
        F: Future<Output = NaviResult<T>>,
    {
    ...
    pub async fn run_waiting<F, U, T>(self: Arc<Self>, f: U) -> NaviResult<T>
    where
        U: FnOnce(Arc<Self>) -> F,
        F: Future<Output = NaviResult<T>>,
    {
    ...
    fn send_payload(&self, payload: EventPayload) -> NaviResult<()> {
        ...
        if let Some(sender) = &self.sender {
            sender
                .send(event)
                .map_err(|e| NaviError::unknown(Box::new(e)))?;
        } else {
...
impl MetaJobHandle {
    /// Runs a closure, automatically updating the job monitor based on the result.
    pub async fn run<F, U, T>(self, f: U) -> NaviResult<T>
    where
