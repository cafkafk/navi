...
use crate::{error::NaviError, nix::HivePath};
...
where
    F: Future<Output = Result<T, NaviError>>,
{
    match f.await {
        Ok(r) => r,
        Err(error) => {
...
fn troubleshoot(hive_config: Option<HivePath>, error: &NaviError) -> Result<(), NaviError> {
    if let NaviError::NoFlakesSupport = error {
        // People following the tutorial might put hive.nix directly
        // in their Navi checkout, and encounter NoFlakesSupport
        // because Navi always prefers flake.nix when it exists.
        ...
        if hive_config.is_none() {
            ...
            if cwd.join("flake.nix").is_file() && cwd.join("hive.nix").is_file() {
                ...
                eprintln!("      Navi will always prefer flake.nix if it exists.");
                eprintln!();
