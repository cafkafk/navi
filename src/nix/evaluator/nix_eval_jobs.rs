...
impl DrvSetEvaluator for NixEvalJobs {
    async fn evaluate(
        ...
        flags: NixFlags,
    ) -> NaviResult<Pin<Box<dyn Stream<Item = EvalResult> + Send>>> {
        ...
        tokio::spawn(async move { capture_stream(stderr, Some(job), true, false).await });
        ...
        Ok(Box::pin(stream! {
            loop {
                ...
                match serde_json::from_str::<EvalLine>(trimmed) {
                    ...
                    Err(e) => {
                        let bad_output = NaviError::BadOutput {
                            output: e.to_string(),
                        };
...
impl From<EvalLineGlobalError> for NaviError {
    fn from(ele: EvalLineGlobalError) -> Self {
        NaviError::Unknown { message: ele.error }
    }
}
...
///
/// This is used for informational purposes in `navi nix-info`.
