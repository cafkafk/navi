...
impl StorePath {
    ...
    pub async fn references(&self) -> NaviResult<Vec<StorePath>> {
    ...
    pub fn into_derivation<T: TryFrom<BuildResult<T>>>(self) -> NaviResult<StoreDerivation<T>> {
        if self.is_derivation() {
            Ok(StoreDerivation::<T>::from_store_path_unchecked(self))
        } else {
            Err(NaviError::NotADerivation { store_path: self })
        }
    }
}
...
impl TryFrom<String> for StorePath {
    ...
    fn try_from(s: String) -> NaviResult<Self> {
        if s.starts_with("/nix/store/") {
            Ok(Self(s.into()))
        } else {
            Err(NaviError::InvalidStorePath)
        }
    }
}
...
impl<T: TryFrom<BuildResult<T>, Error = NaviError>> StoreDerivation<T> {
    /// Builds the store derivation on a host, resulting in a T.
    pub async fn realize(&self, host: &mut Box<dyn Host>) -> NaviResult<T> {
    ...
    pub async fn realize_remote(&self, host: &mut Box<dyn Host>) -> NaviResult<T> {
...
impl<T: TryFrom<BuildResult<T>, Error = NaviError>> BuildResult<T> {
