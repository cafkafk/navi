...
use super::{NaviError, NaviResult, NixCheck};
...
impl Flake {
    ...
    pub async fn from_dir<P: AsRef<Path>>(dir: P) -> NaviResult<Self> {
    ...
    pub async fn from_uri(uri: impl AsRef<str>) -> NaviResult<Self> {
        NixCheck::require_flake_support().await?;
    ...
    ///
    /// NOTE: that the URI will not be locked if the git workspace is dirty.
...
impl FlakeMetadata {
    /// Resolves a flake.
    async fn resolve(flake: &str) -> NaviResult<Self> {
        ...
        serde_json::from_slice::<FlakeMetadata>(&output.stdout).map_err(|_| {
            ...
            NaviError::BadOutput { output }
        })
    }
}

/// Quietly locks the dependencies of a flake.
pub async fn lock_flake_quiet(uri: &str) -> NaviResult<()> {
