...
use super::{BuildResult, Goal, NaviError, NaviResult, StoreDerivation, StorePath};
...
impl Profile {
    pub fn from_store_path(path: StorePath) -> NaviResult<Self> {
        if !path.is_dir() || !path.join("bin/switch-to-configuration").exists() {
            return Err(NaviError::InvalidProfile);
        }

        if path.to_str().is_none() {
            Err(NaviError::InvalidProfile)
        } else {
            Ok(Self(path))
        }
    }
    ...
    pub async fn create_gc_root(&self, path: &Path) -> NaviResult<()> {
...
impl TryFrom<BuildResult<Profile>> for Profile {
    ...
    fn try_from(result: BuildResult<Self>) -> NaviResult<Self> {
        let paths = result.paths();

        if paths.is_empty() {
            return Err(NaviError::BadOutput {
