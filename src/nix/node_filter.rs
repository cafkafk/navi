...
use super::{NaviError, NaviResult, NodeConfig, NodeName};
...
impl NodeFilter {
    ...
    pub fn new<S: AsRef<str>>(filter: S) -> NaviResult<Self> {
        ...
        let rules = trimmed
            ...
            .map(|pattern| {
                ...
                if pattern.is_empty() {
                    return Err(NaviError::EmptyFilterRule);
                }
    ...
    pub fn filter_node_names(&self, nodes: &[NodeName]) -> NaviResult<HashSet<NodeName>> {
        nodes.iter().filter_map(|name| -> Option<NaviResult<NodeName>> {
            for rule in self.rules.iter() {
                match rule {
                    Rule::MatchName(pat) => {
                        if pat.matches(name.as_str()) {
                    ...
                    _ => {
                        return Some(Err(NaviError::Unknown {
