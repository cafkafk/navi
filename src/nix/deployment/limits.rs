...
pub struct ParallelismLimit {
    ...
    pub evaluation: Arc<Semaphore>,

    /// Limit of concurrent apply processes.
    pub apply: Arc<Semaphore>,
}

impl Default for ParallelismLimit {
    fn default() -> Self {
        Self {
            evaluation: Arc::new(Semaphore::new(1)),
            apply: Arc::new(Semaphore::new(10)),
        }
    }
}
...
impl ParallelismLimit {
    ...
    pub fn set_apply_limit(&mut self, limit: usize) {
        self.apply = Arc::new(Semaphore::new(limit));
    }
}
