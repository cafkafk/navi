{
  rawHive ? null, # Navi Hive attrset
  rawFlake ? null, # Nix Flake attrset with `outputs.navi`
...
let
  ...
  uncheckedHive =
    let
      flakeToHive =
        rawFlake:
        if rawFlake.outputs ? navi then rawFlake.outputs.navi else throw "Flake must define outputs.navi.";
    ...
    let
      ...
      mergedHive =
        assert lib.assertMsg (!(uncheckedHive ? __schema)) ''
          ...
          Hint: Use the `naviHive` output instead of `navi`.
        '';
  ...
  evalNode =
    ...
    let
      ...
      # Here we need to merge the configurations in meta.nixpkgs and in machine
      # config.
      nixpkgsModule =
        { config, lib, ... }:
        let
        ...
        in
        {
          ...
          warnings =
            let
              # NOTE: Before 22.11, most config keys were untyped thus the merging was
              # broken. Let's warn the user if not all config attributes set in
    ...
    evalConfig {
      ...
      modules = [
        nixpkgsModule
        naviModules.assertionModule
        naviModules.keyChownModule
        naviModules.keyServiceModule
        naviOptions.deploymentOptions
        hive.defaults
      ] ++ configs;
      specialArgs =
        {
  ...
  # Used as the `nodes` argument in modules. We skip recursive type checking for
  # performance.
...
rec {
  # Exported attributes
  #
  # TODO: Consider how NAVI should interact with this schema, we likely want our own?
  __schema = "v0.5";
  ...
  exportSelectedConfig =
    names:
    nixpkgs.writeText "config.json" (
      builtins.toJSON (
        lib.mapAttrs (_: v: {
          targetHost = v.config.deployment.targetHost;
          targetUser = v.config.deployment.targetUser;
          targetPort = v.config.deployment.targetPort;
          allowLocalDeployment = v.config.deployment.allowLocalDeployment;
          buildOnTarget = v.config.deployment.buildOnTarget;
          tags = v.config.deployment.tags;
          replaceUnknownProfiles = v.config.deployment.replaceUnknownProfiles;
          privilegeEscalationCommand = v.config.deployment.privilegeEscalationCommand;
          sshOptions = v.config.deployment.sshOptions;
          provisioner = v.config.deployment.provisioner;
          providers = v.config.deployment.providers;
          keys = { };
        }) (lib.filterAttrs (name: _: elem name names) nodes)
      )
    );
  metaConfig = intersectAttrs metaConfigKeysSet hive.meta;
  introspect =
    f:
    f {
      inherit lib;
