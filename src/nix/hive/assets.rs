...
impl Assets {
    pub async fn new(hive_path: HivePath) -> NaviResult<Self> {
        let temp_dir = TempFileBuilder::new().prefix("navi-assets-").tempdir()?;
    ...
    pub fn get_base_expression(&self) -> String {
        match &self.hive_path {
            HivePath::Legacy(path) => {
                format!(
                    "with builtins; let eval = import {eval_nix}; hive = eval {{ rawHive = import {path}; naviOptions = import {options_nix}; naviModules = import {modules_nix}; }}; in ",
...
fn create_file(base: &TempDir, name: &str, executable: bool, contents: &[u8]) -> NaviResult<()> {
