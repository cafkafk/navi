...
use super::{Goal, Key, Profile, Provenance, StorePath};
use crate::error::{NaviError, NaviResult};
use crate::job::JobHandle;
use crate::nix::DiskUnlockConfig;

mod ssh;
pub use ssh::Provider;
pub use ssh::Ssh;
...
pub struct RebootOptions {
    ...
    /// Disk unlock configuration.
    pub unlock: Option<DiskUnlockConfig>,
}
...
impl RebootOptions {
    ...
    pub fn unlock(mut self, config: Option<DiskUnlockConfig>) -> Self {
        self.unlock = config;
        self
    }
}
...
pub trait Host: Send + Sync + std::fmt::Debug {
    ...
    async fn realize(&mut self, derivation: &StorePath) -> NaviResult<Vec<StorePath>> {
    ...
    async fn deploy(
        ...
        if goal.requires_activation() {
            self.activate(profile, goal, install_bootloader).await?;
        }
