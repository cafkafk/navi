...
pub async fn run() {
    ...
    if let Command::GenManual = opts.command {
        print_manual();
        return;
    }
...
/// Print the command-line reference as Markdown.
///
/// The output is the source for `docs/src/reference/cli.md`. It is generated
/// from the `Opts` definition so the reference cannot drift from the binary.
/// Regenerate with `navi gen-manual > docs/src/reference/cli.md`.
fn print_manual() {
    ...
    print!(
        ...
        clap_markdown::help_markdown_custom::<Opts>(&options)
    );
}
