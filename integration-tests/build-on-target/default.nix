...
tools.runTest {
  ...
  navi.test = {
    ...
    testScript = ''
      ...
      deployer.succeed("cd /tmp/bundle && ${tools.naviExec} apply --on alpha")
      ...
      with subtest("Check that we can override per-node settings and build locally"):
          deployer.succeed("cd /tmp/bundle && ${tools.naviExec} build --on alpha --no-build-on-target")
      ...
      with subtest("Check that we can override per-node settings and build remotely"):
          deployer.succeed("cd /tmp/bundle && ${tools.naviExec} apply --on beta --build-on-target")
