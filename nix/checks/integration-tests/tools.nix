...
# deployer is where navi will run.
#
# TODO: Modularize most of this

{
  insideVm ? false,
  deployers ? [ "deployer" ], # Nodes configured as deployers (with Navi and pre-built system closure)
  targets ? [
    "alpha"
    "beta"
    "gamma"
  ...
  navi ? if !insideVm then pkgs.navi else throw "Cannot eval inside VM",
}:
...
let
  ...
  ## The modular NixOS test framework with Navi additions
  naviTestModule =
    { lib, config, ... }:
    let
      cfg = config.navi.test;
        ...
        navi.test = {
          ...
          testScript = lib.mkOption {
            description = ''
              ...
              The Navi test framework will prepend initialization
  ...
  commonConfig =
    { pkgs, ... }:
    {
      # Temporary workaround for <https://github.com/zhaofengli/navi/issues/281>
      # References:
  ...
  # A "shallow" re-evaluation of the test for use from Navi
